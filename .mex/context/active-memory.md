...
PR5 Batch 003 / 001 is in progress on branch `pr5-batch003-001-terrain-shell-v2`. The patch converts `AJungleFullSizeTerrainShellActor` from cube-only blockout to a deterministic procedural mesh heightfield with basin, creek-valley, ridge-spine, mountain-shoulder, coast, and ocean-edge shaping; retains cube blockout as explicit debug fallback via `-JungleDebugCubeShell`; adds the `ProceduralMeshComponent` module/plugin; and adds `-JungleTerrainShellSmoke` so HighResShot can validate the terrain substrate instead of the Cell 0 cube slice. Validation so far: UBT `JungleGameEditor Linux Development` succeeded after the procedural terrain patch and again after the visible-relief/normals patch; live runtime emitted `JG_FULL_TERRAIN_SHELL_003`; user visual inspection confirmed the player could stand on the procedural terrain and that the later pass placed the player on a visibly sloped hill. Automatic `-JungleTerrainShellSmoke` requested HighResShot but exited before a screenshot file was written, so manual/live visual validation is the current evidence. Next action is to stage intended files only and commit; later smoke timing can be adjusted if automated screenshot evidence is required.
