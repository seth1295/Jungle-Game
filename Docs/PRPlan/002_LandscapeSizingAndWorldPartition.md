...
The large-world first playable targets a 16 km x 16 km rainforest/mountain island landscape, roughly 256 km2. Ocean is the natural world boundary; invisible walls must not be the primary edge solution.
...
- Coastline, creek mouths, headlands, cliff coasts, mangroves, rocky shores, beaches, and difficult coastal terrain must be represented as landform/ecosystem edge cases, not as a square technical border.
...
- authored shelter, smoke, or first-playable integration layers only when they are real gameplay state.
...
- island/ocean boundary assumption documented;
